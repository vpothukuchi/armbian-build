...
function prepare_python_and_pip() {
	...
	if [[ -f "${python_hash_file}" ]]; then
        ...
        run_host_command_logged env -i HTTPS_PROXY=http://172.16.172.135:80 "${PYTHON3_VARS[@]@Q}" "${PYTHON3_INFO[BIN]}" "${PYTHON3_INFO[GET_PIP_BIN]}" "${pip3_extra_args[@]}" "pip==${pip3_version_number}"

		# Install the dependencies
		display_alert "Installing Python dependencies" "from ${python3_pip_dependencies_path}" "info"
        run_host_command_logged env -i HTTPS_PROXY=http://172.16.172.135:80 "${PYTHON3_VARS[@]@Q}" "${PYTHON3_INFO[BIN]}" -m pip install "${pip3_extra_args[@]}" -r "${python3_pip_dependencies_path}"
