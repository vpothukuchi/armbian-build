...
#   A1/A2/A3 have no dependency on B1 either.
...
#   B2 consumes all outputs: B1 kernel/uboot + all EdgeAI .deb files.
#
# Proxy setup runs once before the first compile.sh invocation (idempotent).
# All EdgeAI .deb files are staged into output/debs/extra/ before B2.
#
# ============================================================================
...
# Usage examples:
#
#   *** FULL BUILD — single command (runs all 5 phases): ***
...
#
#   # Full build, vision-apps from prebuilt Yocto IPKs instead of source:
#   bash packages/edgeai/build_armbian.sh --ipk-dir /path/to/yocto/ipk
#
...
#   --ipk-dir <path>        Prebuilt Yocto IPK directory for ti-vision-apps (Docker only)
...
# vision-apps needs --sdk-path or --ipk-dir
if [[ "${SKIP_VISION_APPS}" -eq 0 && -z "${SDK_PATH}" && -z "${IPK_DIR}" ]]; then
    echo "[WARN] Neither --sdk-path nor --ipk-dir provided — skipping A2 (ti-vision-apps) and A3 (ti-tidl)."
...
stage_debs() {
    echo ""
    echo "--- Staging EdgeAI .deb files for Armbian ---"
    ...
    if [[ "${USE_DOCKER}" -eq 1 ]]; then
        ...
        if [[ -n "${IPK_DIR}" ]]; then
            bash "${DOCKER_BUILD}" "${DOCKER_ARGS[@]+"${DOCKER_ARGS[@]}"}" \
                --prebuilt --ipk-dir "${IPK_DIR}" ti-vision-apps
        else
            bash "${DOCKER_BUILD}" "${DOCKER_ARGS[@]+"${DOCKER_ARGS[@]}"}" \
